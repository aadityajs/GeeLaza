...
<div style="width:685px; float: left; margin: 0  0 0 8px;">
	...
	<?php
		...
		if ($orders_num <= 0) {
	?>
		<p style="font-family: Helvetica; font-size: 16px;">You haven&rsquo;t bought a deal yet.</p>
		<p style="font-family: Helvetica; font-size: 12px;">You haven&rsquo;t made any purchases yet or all of your deals have  been used, redeemed, or given as gifts. To see a full record of all the  vouchers you've used or gifted in the past, visit your&nbsp;<a href="#">Purchase History</a>.</p>
	<?php 		
		}
		else {
...
?>
	...
	<div class="TabbedPanels1" id="myaccount_2" style="display:none;">
	   <div style="width:76%;"  class="title">
		 ...
		 <span class="float_right" style="font-size:12px; font: bold 12px/29px Arial, Helvetica, sans-serif; margin:-10px -130px 0px 0;">My GeeLaza credits</span>
		<div class="clear"></div>
	  </div>
  ...
  <table width="450" border="0" align="left" cellpadding="3" cellspacing="3">
	...
	if(strtolower($_SERVER['REQUEST_METHOD']) == 'post' && $_POST["btnEmailUpdate"]=='Save')
	{
		...
		if($flag != 1) {	// && $current_password == base64_encode($epassword)
				...
				$succ_msg = 'Email address chnages has been saved.';
				//$flag =2;
	...
	if(strtolower($_SERVER['REQUEST_METHOD']) == 'post' && $_POST["btnPhnoUpdate"]=='Save')
	{
		...
		if($flag != 1) {		// && $current_password == base64_encode($phpassword)
				...
				mysql_query($sql_email_update);
				$succ_msg = 'Phone no. changes has been saved.';
				//$flag =2;
	...
	<table width="1005" border="0" cellspacing="0" cellpadding="0" class="edit_table">
     ...
     <table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
		...
		<table class="account_table" id="pass">
      	...
      	<table class="account_table" id="unsubscribe">
			<tr>
				<td colspan="3"><a onclick="javascript:  UnsubAll();" style="cursor: pointer;"><b>Unsubscribe from all GeeLaza email</b></a></td>
				<div id="unsubs_msg_loc"></div>
			</tr>
      	</table>
      	
      	<form action="<?php echo SITE_URL.'customer-account.php?usucc=You unsubscribed from all cities&tab=myaccount'; ?>" method="post" name="frm_unsub_all">
...
function passMatch() {
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="purchase_history">
      ...
      <tr>
      	<td style="border: none;"><input type="button" name="cancel" class="buyu_btn" value="Done to Subscribe" onclick="cancelFields(this); return false;"/></td>
      </tr>      
    </table>

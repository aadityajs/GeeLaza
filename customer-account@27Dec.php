...
<div style="width:685px; float: left; margin: 0  0 0 8px;">
   	...
   	<div class="tabs">
		...
		<a href="javascript: show_tab(6);" id="tab_6">Purchage history</a>
		<a href="javascript: show_tab(2);" id="tab_2">Credits</a>
		<a href="javascript: show_tab(4);" id="tab_4">Royals Points</a>
	...
	<!-- loop start -->		
	   ...
	   <!-- loop ends -->		
<?php } ?>
	<?php if ($orders_num <= 0) {?><p><b>You still have not bought a deal</b></p><?php } ?>


    </div>	<!-- 1 ends here  -->

	<div class="TabbedPanels1" id="myaccount_2" style="display:none;">
	   <div style="width:76%;"  class="title">
		 <span class="float_left">Credit</span>	  	 		
		<div class="clear"></div>
	  </div>
      <div style="float:right; height:29px; margin:-26px 0 0 0; padding:0px; width:150px; text-align:center; font: bold 12px/29px Arial, Helvetica, sans-serif;  background: url(images/tab_bg1.gif) left top repeat-x;">
		  Credits: &pound;10
		</div>
		...
		be sure to tick the box to use your credit when you checkout.
		 ...
		 <h3 style="text-align:left;"><span style="float:left; line-height:32px; padding-right:5px;">consectetur, adipisci velit, sed quia non numquam eius</span><img src="images/recomandedus.png" alt="" /></h3>  
    </div><!-- 2 ends here  -->
	...
	<div class="TabbedPanels1" id="myaccount_3" style="display:none;">
		 <div class="title">Account
				...
				<!-- <img src="images/tick_mark.gif" align="top" style=" float:left; margin:0 5px 0 0;"> --><?php echo $_SESSION['succ_msg']?>
				</label>
...
//
	...
	if(strtolower($_SERVER['REQUEST_METHOD']) == 'post' && $_POST["btnNameUpdate"]=='Save')
	{
		...
		//if($flag != 1 && $current_password == base64_encode($npassword)) {
				...
				$_SESSION['succ_msg'] = 'Customer updated successfully';
   ...
   <!-- <?php
		...
		?>
			<div style="height: 25px; width: auto; margin: 10px; padding: 5px; border: 1px dashed <?php if ($flag == 1) {echo "red";} else {echo "green"; } ?>;">
				...
				<?php } ?>
				<?php if($_SESSION['succ_msg'] != "") { ?>
				<label style="color: #006600; font-weight: bold;"><?=$_SESSION['succ_msg']?></label>
				<?php }	?>
			</div>
		<?php }	?> -->
	...
	if(strtolower($_SERVER['REQUEST_METHOD']) == 'post' && $_POST["btnAddressUpdate"]=='Save')
	{
		...
		if($flag != 1 && $current_password == base64_encode($apassword)) {
				...
				$_SESSION['succ_msg'] = 'Address successfully updated';
				//$flag =2;
  ...
  <table width="450" border="0" align="left" cellpadding="3" cellspacing="3">
	...
	if(strtolower($_SERVER['REQUEST_METHOD']) == 'post' && $_POST["btnEmailUpdate"]=='Save')
	{
		...
		if($flag != 1) {	// && $current_password == base64_encode($epassword)
				...
				$_SESSION['succ_msg'] = 'Email successfully updated';
				$flag =2;
	...
	if(strtolower($_SERVER['REQUEST_METHOD']) == 'post' && $_POST["btnPhnoUpdate"]=='Save')
	{
		...
		if($flag != 1) {		// && $current_password == base64_encode($phpassword)
				...
				mysql_query($sql_email_update);
				$_SESSION['succ_msg'] = 'Phone no. successfully updated';
				$flag =2;
  ...
  <?php 
	...
	if(strtolower($_SERVER['REQUEST_METHOD']) == 'post' && $_POST["btnPassUpdate"]=='Save')
	{
		...
		if($flag != 1 && $err_flag != 1) {	// && $current_password == base64_encode($ppassword)
				...
				$_SESSION['succ_msg'] = 'Password successfully updated';
				$flag =2;
...
function passMatch() {
	...
	<div class="TabbedPanels1" id="myaccount_4" style="display:none;">
		<div class="title"><span class="float_left">Royal Points</span> <span class="float_right" style="font-size:15px;"> <img alt="" src="images/star.png" height="18" width="17" >&nbsp;Royal Points: <strong style="color:#217CED;">0</strong></span></div>
		
		<div>
          <h3 style="text-align:left;">Denouncing pleasure and praising pain was born</h3> 
          <p><strong>dolores eos qui ratione voluptatem sequi nesciunt.</strong><br />
          ...
          <p>            <strong>Mission  Statement</strong><br />
            aliquam quaerat voluptatem. Ut enim ad minima veniam, quis nostrum exercitationem ullam corporis vel illum Babu,<br />
            But I must explain to you how all this mistaken idea of denouncing pleasure and praising pain was born and I will give you a complete account of the system, and expound the actual teachings of the great explorer of the truth, the master-builder of human happiness. No one rejects, dislikes, or avoids pleasure itself, because it is pleasure, but because those who do </p>
          <p>
           <strong>Mission  Statement</strong><br />
          Rationally encounter consequences that are extremely painful. Nor again is there anyone who loves or pursues or desires to obtain pain of itself, because it is pain, but because occasionally circumstances occur in which toil and pain can procure him some great pleasure.</p>
	...
	<!--<table class="myaccount" id="noti">
     ...
     --><table class="myaccount" id="noti">
			...
			<?php 
			$show_city = mysql_query("SELECT * FROM ".TABLE_USER_SUBSCRIPTION." WHERE user_id = $user_id");
			...
			<tr>
				...
				<td style="float:left; width: 400px; margin: 0 auto;"><p>
				<?php while ($show_city_list = mysql_fetch_array($show_city)) {
						$country = 225;
						$sql_city = "SELECT * FROM ".TABLE_CITIES."  WHERE country_id = $country AND city_id = $show_city_list[city_id] GROUP BY city_name ASC";
						$result_city = mysql_query($sql_city);
						$row_city_1 = mysql_fetch_array($result_city);
					?>
					
					<?php echo $row_city_1['city_name'].', '; ?>
				<?php } ?>
				</p></td>
				<td style="float:right; width: 30px; margin: 0 30px;"><p><a onclick="editFields(this); return false;" href="">Edit</a></p></td>
			</tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="purchase_history">
      <tr>
        <td>
           ...
           <div><input type="checkbox" name="checkbox" id="checkbox" /> National Deal</div>
        </td>
      </tr>
      <tr>
      	<td>
		...
		?>
			...
			<div class="deal_list">
				<input type="checkbox" name="<?php echo $row_city["city_name"]; ?>" id="<?php echo $row_city["city_name"]; ?>" value="<?php echo $row_city["city_id"]; ?>" onclick="javascript: return ajaxReq(this.value);" <?php if ($get_city >= 1) echo 'checked readonly' ?>/>
      ...
      <tr>
      	<td style="border: none;"><input type="button" name="cancel" class="cancel_btn" value="Done" onclick="cancelFields(this); return false;"/></td>
      </tr>      
    </table>
...
<div class="clear"></div>
		...
		?>
			<div style="height: 40px; width: auto; margin: 10px; margin-left: 120px; margin-right: 5px; padding: 10px 10px 0 10px; border: 1px solid <?php if ($err_msg != "") {echo "none";} else {echo "none"; } ?>;">
				...
				<?php } ?>
				<?php if($_SESSION['succ_msg'] != "") { ?>
				<label style="color: #191919; font: bold 20px/26px Arial, Helvetica, sans-serif;"><img src="images/tick_mark.gif" align="top" style=" float:left; margin:0 5px 0 0;"><?=$_SESSION['succ_msg']?></label>
				<?php }	?>
			</div>
		<?php } ?>-->
